...
export default function ViagensApp() {
  ...
  async function deleteViagem(id) {
    ...
    if (!window.confirm("Confirma exclusão desta viagem?")) return;
    ...
    try {
      ...
      if (!res.ok) {
        const text = await res.text();
        throw new Error(`Erro ao apagar: ${res.status} ${text}`);
      }
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow p-6">
        ...
        <form onSubmit={handleCreate} className="space-y-3 mb-6">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={fetchViagens} className="px-4 py-2 bg-gray-200 rounded">
              Recarregar
            </button>
          </div>
        </form>
              ...
              {viagens.map((s) => (
                <li key={s.id} className="p-3 border rounded">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="font-semibold">{s.origem}</div>
                      <div className="font-semibold">{s.destino}</div>
